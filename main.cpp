...
bool Inicio(string ruta,string denominacion)
{
	...
	for (int i = 0; i < intentos ; i++)
    {
        ...
        cout<<"\t 2.- Clave: ";
        cin>>clave;
        if (busqueda_inicio(usuario,clave,arr))
            {
...
void registrarp()
{
	...
	for(i=0;i<np;i++)
	{
		...
		cout<<"\t C�digo: ";cin>>(regPe+i)->codigop;
...
{/*
	...
	do
	{
		...
		system("cls");
		cout<<"\t\t\t ***INTRALOT*** \t\t\t"<<endl<<endl;
		cout<<"\t\t Inicio de Sesi�n del Administrado \t\t"<<endl<<endl;
		cout<<"\t 1.- Usuario: ";
		while(usua2=getch())
		{
				if(usua2==13)
	...
	{
		cout<<endl;
		cout<<"\t Ha excedido el n�mero m�ximo de intentos"<<endl<<endl;
	}
	*/
}
void mostrarp()
