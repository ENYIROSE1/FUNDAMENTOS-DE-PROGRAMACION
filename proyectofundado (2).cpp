...
void ingresos()
{
    ...
    gotoxy(23,17);
  	...
  	switch(opcion){
        ...
        system("cls");
        gotoxy(15,10);
  			system("color 71");printf("Ingreso de empleados omitido por inicio de logueo");
  			getch();
  			break;
  		}
  		case 3:{
...
void productosinventario()
{
    ...
    while(!consultapi.eof())
    {
      	...
      	if(ingreso.cod==ingreso.auxcod)
      	{
            cout<<endl;
            gotoxy(16,10);
  	        system("color 71");printf("Ya existe un Producto con ese Codigo");
  	        repetido=true;
            getch();
  	        break;
      	}
    ...
    if(repetido==false)
    {
        ...
        if(si==false){
            gotoxy(16,16);
            system("color 71");printf("Proveedor No Encontrado, Ingrese e Intentelo de Nuevo");
            productos.close();
            consultap.close();
            consultapi.close();
            getch();
...
void ingresolog()
{
    ...
    cout<<endl<<endl;
    cout<<"*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-"<<endl;
    system("color 71");printf("NOTA: el Codigo y el Usuario No se podran modificar en un futuro");
    cout<<endl<<endl;
    ...
    while(!consultal.eof())
    {
      	...
      	if((login.codigo==login.auxcodigo)|(strcmp(login.user, login.auxuser) == 0))
      	{
        	system("cls");
        	gotoxy(20,10);
        	system("color 71");printf("Lo sentimos, Codigo o Usuario ya en Uso");
        	repetido=true;
        	log.close();
        	consultal.close();
        	break;
...
{
    system("cls");
    cout<<"DERECHOS FUNDAMENTOS DE PROGRAMACION"<<endl;
